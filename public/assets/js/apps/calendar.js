...
!function (NioApp, $) {
  ...
  NioApp.Calendar = function () {
    ...
    var TODAY = yyyy + '-' + mm + '-' + dd + 'T' + checkTime(today.getHours()) + ":" + checkTime(today.getMinutes()) + ":" + checkTime(today.getSeconds());
    ...
    $('.currentDate').text(monthNames[date.getMonth()] + " " + date.getDate());
    $('#eventLeftCalendar').datepicker('setDate', today);
    $("#accordion").show();
    $('#eventLeftCalendar').on('changeDate', function (e) {
      var formartSelectedDate = formatDate($('#eventLeftCalendar').datepicker('getFormattedDate'));
      calendar.changeView('timeGridDay', formartSelectedDate);
      $('#hidden_eventLeftCalendar').val($('#eventLeftCalendar').datepicker('getFormattedDate'));
